...
namespace _63_NetworkStreamMultiThreadServerForm {
    public partial class Form1 : Form {
        ...
        string baseball(string userNum) {
            int strike = 0, ball = 0;
            if(userNum.Length < 3)
                return "서로 다른 정수 3개를 입력해주세요";
            for(int i = 0; i < 3; i++) {
                if('0'>userNum[i] || userNum[i] > '9') {
                    return "서로 다른 정수 3개를 입력해주세요";
                }
            }
            for(int i = 0; i < 3; i++) {
            ...
            return strike.ToString() + "s" + ball.ToString() + "b";
        }
        void ThreadRecv(object obj) {
            ...
            //StreamWriter sw = new StreamWriter(ns);
            SendAllChat(socket, "서로 다른 정수 3개를 입력해주세요");
            while(this.isRecv) {
                try {
                    string data = sr.ReadLine();
                    AddLogListBox("← Client 수신 : " + data);
                    string baseballResult = baseball(data);
                    SendAllChat(socket, baseballResult);
                    if(baseballResult[0] == '3')
                        SendAllChat(socket, "strike out");
                    AddLogListBox("→ Client Echo : " + baseballResult);
